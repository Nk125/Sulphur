...
public:
    ...
    std::string query_to_string(Query q) {
        std::string buf = "?";

        for (Url::KeyVal kv : q) {
            std::string k = kv.key(), v = kv.val();

            buf.append(k + (v.empty() ? "" : "=" + v + "&"));
        }

        buf = buf.substr(buf.size() - 1); // Removes ? if not founds anything and removes the last &
